...
class SSOApplicationReconciler(Reconciler):
    ...
    def reconcile(self, spec, meta, status, patch, name, namespace, logger, **_) -> None:
        ...
        if protocol == PROTOCOL_PROXY:
            provider_pk = self._reconcile_proxy(
                ak,
                cfg,
                spec,
                slug,
            ...
            provider_pk, client_id, openbao_path = self._reconcile_oauth2(
                ak,
                cfg,
                bao,
                spec,
    ...
    def _reconcile_oauth2(
        self, ak, cfg, bao, spec, slug, authz_pk, inval_pk, mapping_pks, patch, status, generation
    ) -> tuple[int, str, str]:
    ...
    def _reconcile_proxy(
        self, ak, cfg, spec, slug, authz_pk, inval_pk, mapping_pks, patch, status, generation
    ) -> int:
    ...
    def finalize(self, spec, status, name, logger, **_) -> None:
...
_reconciler = SSOApplicationReconciler()


@kopf.on.create(
    SSOApplicationReconciler.GROUP,
    SSOApplicationReconciler.VERSION,
    SSOApplicationReconciler.plural,
)
@kopf.on.update(
    SSOApplicationReconciler.GROUP,
    SSOApplicationReconciler.VERSION,
