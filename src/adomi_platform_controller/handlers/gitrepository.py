...
class GitRepositoryReconciler(Reconciler):
    ...
    def reconcile(self, spec, meta, status, patch, name, namespace, **_) -> None:
        ...
        if preview.get("enabled"):
            webhook_url = self._reconcile_preview(
                cfg,
                spec,
                preview,
                owner,
            ...
            self._delete_preview(cfg, owner, repo)
    ...
    def _reconcile_preview(
        self, cfg, spec, preview, owner, repo, name, namespace, patch, status, generation
    ) -> str:
        ...
        try:
            ...
            buildsecrets.ManagedSecret.token(token_secret, cfg.argo_namespace, token).apply()
            # Generate the HMAC secret once; never rotate it from under the webhook.
            buildsecrets.ManagedSecret.opaque(
                webhook_secret,
                cfg.argo_namespace,
            ...
            argoevents.apply_eventsource(
                argoevents.EventSourceSpec(
                    ...
                    labels=self._labels(owner, repo),
                )
            )
    ...
    def finalize(self, spec, status, logger, **_) -> None:
    ...
    def _delete_preview(self, cfg, owner: str, repo: str) -> None:
        ...
        ingress.IngressRoute.delete(es_name, cfg.argo_namespace)
        buildsecrets.ManagedSecret.delete(f"{es_name}-token", cfg.argo_namespace)
        buildsecrets.ManagedSecret.delete(f"{es_name}-webhook", cfg.argo_namespace)

    @staticmethod
    def _labels(owner: str, repo: str) -> dict[str, str]:
        return {
            "app.kubernetes.io/managed-by": GitRepositoryReconciler.MANAGED_BY,
            "platform.adomi.io/repo": f"{owner}-{repo}".lower(),
        }


_reconciler = GitRepositoryReconciler()


@kopf.on.create(
    GitRepositoryReconciler.GROUP, GitRepositoryReconciler.VERSION, GitRepositoryReconciler.plural
)
@kopf.on.update(
    GitRepositoryReconciler.GROUP, GitRepositoryReconciler.VERSION, GitRepositoryReconciler.plural
)
